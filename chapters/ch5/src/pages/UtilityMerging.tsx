...
export default function UtilityMergingPage() {
  ...
  const results = useMemo(() => {
    return alternatives.map((alt) => {
      ...
      switch (activeRule) {
        ...
        case 'additive': {
          W = u.reduce((sum, ui, i) => sum + weights[i] * ui, 0);
          break;
        }
        case 'multiplicative': {
          W = u.reduce((prod, ui, i) => prod * Math.pow(ui, rhos[i]), 1);
          break;
        }
        case 'mixed': {
          if (Math.abs(gamma) < 0.0001) {
            W = u.reduce((sum, ui, i) => sum + mixedWeights[i] * ui, 0);
          } else {
            const prod = u.reduce(
              (p, ui, i) => p * (1 + gamma * mixedWeights[i] * ui),
              1
            );
            W = (prod - 1) / gamma;
          }
  ...
  }, [alternatives, activeRule, weights, rhos, gamma, mixedWeights]);
  ...
  const weightSum = useMemo(
    () => (activeRule === 'additive' ? weights.reduce((a, b) => a + b, 0) : 0),
    [weights, activeRule]
  );
  const rhoSum = useMemo(
    () => (activeRule === 'multiplicative' ? rhos.reduce((a, b) => a + b, 0) : 0),
    [rhos, activeRule]
  );
  /* ────────────── chart data ────────────── */
  ...
  const calcSteps = useMemo<CalcStep[]>(() => {
    ...
    if (activeRule === 'distance') {
      ...
      steps.push({
        title: 'Step 1: 确认权重系数',
        formula: `ρ₁=${weights[0].toFixed(2)}, ρ₂=${weights[1].toFixed(2)}, ρ₃=${weights[2].toFixed(2)}, ρ₄=${weights[3].toFixed(2)}  |  Σρᵢ = ${weightSum.toFixed(2)}${Math.abs(weightSum - 1) < 0.01 ? ' ✓' : ' ⚠ 权重之和不等于1'}`,
        highlight: true,
      });
      results.forEach((r, idx) => {
        const alt = alternatives[idx];
        const terms = [alt.g1, alt.g2, alt.g3, alt.g4].map(
          (u, i) => `${weights[i].toFixed(2)}×${u.toFixed(2)}`
        );
        const vals = [alt.g1, alt.g2, alt.g3, alt.g4].map(
          (u, i) => (weights[i] * u).toFixed(3)
        );
        const isOptimal = idx === optimalIndex;
        steps.push({
          title: `Step ${idx + 2}: 方案${alt.name} 综合效用计算`,
      ...
      steps.push({
        ...
        formula: `ρ₁=${rhos[0].toFixed(2)}, ρ₂=${rhos[1].toFixed(2)}, ρ₃=${rhos[2].toFixed(2)}, ρ₄=${rhos[3].toFixed(2)}  |  Σρᵢ = ${rhoSum.toFixed(2)}${Math.abs(rhoSum - 1) < 0.01 ? ' ✓' : ' ⚠ 之和不等于1'}`,
        highlight: true,
      });
      results.forEach((r, idx) => {
        const alt = alternatives[idx];
        const terms = [alt.g1, alt.g2, alt.g3, alt.g4].map(
          (u, i) => `${u.toFixed(2)}^${rhos[i].toFixed(2)}`
        );
        const vals = [alt.g1, alt.g2, alt.g3, alt.g4].map((u, i) =>
          Math.pow(u, rhos[i]).toFixed(4)
        );
        const isOptimal = idx === optimalIndex;
        steps.push({
          title: `Step ${idx + 2}: 方案${alt.name} 综合效用计算`,
      ...
      steps.push({
        ...
        formula: `γ = ${gamma.toFixed(2)}, c₁=${mixedWeights[0].toFixed(2)}, c₂=${mixedWeights[1].toFixed(2)}, c₃=${mixedWeights[2].toFixed(2)}, c₄=${mixedWeights[3].toFixed(2)}`,
        highlight: true,
      });
      results.forEach((r, idx) => {
        const alt = alternatives[idx];
        const terms = [alt.g1, alt.g2, alt.g3, alt.g4].map(
          (u, i) => `(1 + ${gamma.toFixed(1)}×${mixedWeights[i].toFixed(2)}×${u.toFixed(2)})`
        );
        const vals = [alt.g1, alt.g2, alt.g3, alt.g4].map(
          (u, i) => (1 + gamma * mixedWeights[i] * u).toFixed(4)
        );
    ...
    return steps;
  }, [activeRule, weights, weightSum, rhos, rhoSum, gamma, mixedWeights, results, alternatives, optimalIndex]);
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {activeRule === 'additive' && (
                      <span
                        className="text-xs"
                        style={{
                          color: Math.abs(weightSum - 1) < 0.01 ? '#4CAF50' : '#ef4444',
                        }}
                      >
                        {Math.abs(weightSum - 1) < 0.01
                          ? `Σ = ${weightSum.toFixed(2)} ✓`
                          : `Σ = ${weightSum.toFixed(2)} ⚠ 权重之和须等于1`}
                      </span>
                    )}
                    {activeRule === 'multiplicative' && (
                      <span
                        className="text-xs"
                        style={{
                          color: Math.abs(rhoSum - 1) < 0.01 ? '#4CAF50' : '#ef4444',
                        }}
                      >
                        {Math.abs(rhoSum - 1) < 0.01
                          ? `Σ = ${rhoSum.toFixed(2)} ✓`
                          : `Σ = ${rhoSum.toFixed(2)} ⚠ 指数之和须等于1`}
                      </span>
                    )}
                  </div>
