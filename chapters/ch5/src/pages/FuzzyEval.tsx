...
export default function FuzzyEvalPage() {
  ...
  const pills = [
    ...
    { label: '5.5', path: '/fuzzy-eval', text: '模糊综合评价' },
  ];
  ...
  return (
    <Layout>
      <div className="px-4 md:px-6 py-6 max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {pills.map((p) => (
            <a
              ...
              style={{
                background: p.label === '5.5' ? '#1B3A5F' : '#fff',
                color: p.label === '5.5' ? '#fff' : '#6B6B6B',
                border: p.label === '5.5' ? '1px solid #1B3A5F' : '1px solid #E0DDD5',
              }}
            >
              {p.label} {p.text}
            </a>
        ...
        {/* Breadcrumb */}
        <Breadcrumb
          items={[{ label: '首页', path: '/' }, { label: '5.5 模糊综合评价' }]}
        />
